...
public class Table {
	...
	public void update(String[] columns, String[] values, Condition where) throws SQLException {
		for(Row row: rows){
			...
			{
				row.data.put(columns[0], values[0]);
			}
		}
	}
	
	public void delete(Condition where) throws SQLException {
		List<Row> list_delete = new LinkedList<Row>();
		List<Row> list_notdelete = new LinkedList<Row>();
		for (Row row : rows) {
			if (row.satisfies(where, description)) 
			{
				list_delete.insertLast(row);
			}
			else
			{
				list_notdelete.insertLast(row);
			}
		rows=list_notdelete;	
		}
